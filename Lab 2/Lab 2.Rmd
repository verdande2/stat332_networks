...
Well, taking an initial look at the network graph picture, I see 3 legends at 3 of the 4 corners (missing top left), showing:
 - legend for node size to top right, ranging from min to max of values
 - legend for node color to bottom right, categorical based on 4 different birthplace values
 ...
 - legend titles are centered in their bounding boxes
 - nodes appear to have a semitransparent opacity set
 - edges also appear to have some transparency set
 - looking to margins, it seems that outer margins are zero'd, and inner margins are minimal, so starting with 0
...
gangs <- read_rds("London Gang Network.rds")
```
...
```{r prob4_plot_the_ugly_network}
plot(gangs)
```
Well, I see some circles and numbers and lines, looks about right to me! That was easy enough... On to the plot code...
...
library(RColorBrewer) # color pallettes etc
...
# VERTEX COLORS 
# ------------------------
# - nodes/vertices are colored by "Birthplace (v/n)"
...
v_vals   <- V(gangs)$Birthplace
v_levels <- sort(unique(v_vals)) # will look like c("1", "2", "3", "4")

# keeping the rest of this as is for now... Paired pallette is fine, only choosing four colors anyway...
...
#  VERTEX SHAPES
# ------------------------
# All vertices are displayed as colored circles in the network graph, so this part will be largely unused/commented out
# shape_levels <- levels(as.factor(v_vals))
# shape_pal <- setNames(c("circle", "square", "rectangle")[seq_along(shape_levels)], shape_levels)
# vertex_shapes <- shape_pal[v_vals]

vertex_shapes <- c("circle") # let the vertices be circles, and circles they shall be!
...
#  - edge width is the edge weights ("Weight")

# keeping this to use the weights to set width/size/etc
e_vals   <- E(gangs)$weight # Lesson learned. w != W...
...
edge_colors <- c("#000000") # back in black...
...
#  EDGE LINE TYPES (discrete)
# ------------------------
# commenting out most of this, just setting the line types all to the default solid line == 1
# e_lty_vals   <- E(g)$type
# e_lty_levels <- sort(unique(e_lty_vals))
# 
# # assign linetypes to the levels
...
png("LondonGangNetwork.png", width = 800, height = 600) # saving to file
# ------------------------
# 5. PLOT
# ------------------------
plot(
  ...
  vertex.size = scales::rescale(V(gangs)$Arrests, to = c(1, 25)), # even with 0 arrests, min size is 1 so we still see vertex
  
  # - nodes appear to have a semitransparent opacity set
  vertex.color = adjustcolor(vertex_colors, alpha.f = 0.5), # VERTEX ALPHA HERE
  vertex.shape = vertex_shapes,
  ...
  edge.width = 3 * scales::rescale(E(gangs)$weight, to = c(min(E(gangs)$weight), max(E(gangs)$weight))), # scale factor of 3? Seems enough
  
  #  - edges also appear to have some transparency set
  edge.color = adjustcolor(edge_colors, alpha.f = 0.7), # EDGE ALPHA HERE
  edge.lty = edge_line_types,
...
# see https://stackoverflow.com/a/47308865
 # - legend for node size to top right, ranging from min to max of values
arrest_count_cutoffs <- seq(from = 0,
...
legend_TR_base <- legend(
  "topright",
  title = "Node Arrests",
  legend = arrest_count_cutoffs, # seq(from = min(V(gangs)$Arrests), to = max(V(gangs)$Arrests), length.out = 5), # should have min, max and 5-2 = 3 evenly spaced intermediate sizes for the legend
  pt.cex = scales::rescale(
    arrest_count_cutoffs,
    to = c(0, 1) # normalize to betwen 0-1
  ),
  bty = "n", # TODO look into what this param does in src
  col = "black"
)
legend_TR_circles <- legend(
  "topright",
  legend = arrest_count_cutoffs,
  pt.cex = arrest_count_cutoffs,
  col = "white",
  pch = 21, # circle
  pt.bg = "white"
)
x <- (legend_TR_circles$text$x + legend_TR_circles$rect$left) / 2
y <- legend_TR_circles$text$y
symbols(x, y, circles = arrest_count_cutoffs / 200, inches = FALSE, add = TRUE, bg = "orange")
...
shape_pch_map <- c(
  circle     = 21 # only circles for this network
)
...
legend(
  "bottomleft",
  title = "Edge Weight",
  legend = sort(unique(e_vals)), # should be edge values (=weights) (do I need sort/unique calls on a noncategorical var?)
  pch    = 21, # shape_pch_map[ shape_pal[shape_levels] ], just swapped to 21 for the circle symbol
  pt.bg  = "gray80",
  bty = "n",
  cex = 0.8
)
...
legend(
  ...
  legend = sort(unique(v_vals)), # v_vals col should be birthplace str == one of c("1", "2", ...)
  #col    = e_pal[e_levels], # TODO determine wtf this does
  lwd    = 3, # TODO what do you do?
  bty = "n",
  cex = 0.8
)
