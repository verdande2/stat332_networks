...
plot(
  ...
  vertex.sizes = scales::rescale(V(gangs)$Arrests, to = c(1, 25)), # even with 0 arrests, min size is 1 so we still see vertex
  ...
  edge.widths = 3 * scales::rescale(E(gangs)$weight, to = c(min(E(gangs)$weight), max(E(gangs)$weight))), # scale factor of 3?
...
legend(
  "topright",
  title = "Node Arrests",
  legend = seq(from = min(V(gangs)$Arrests), to = max(V(gangs)$Arrests), length.out = 5), # should have min, max and 5-2 = 3 evenly spaced intermediate sizes for the legend
  border = NA,
  bty = "n",
  cex = 0.8
)
