...
# TODO
# - legend node size bullet position is correct, but radii is too large and causing overlap
# - remove the outline/border of the first/original node size legend in top right
# - legend for edge weight needs a factor of 3 to match the igraph
# - legend for edge weight needs bullets to match thickness of edge.size
# - legend for birthplace bullets need colors
# - legend for birthplace bullets - need different shape, square with fill?
# - need to increase opacity for nodes
# - need to increase plot display size
# - need to decrease size of all legends
# - need to increase spacing between nodes - need to figure out how the fuck to do so...
...
SKIP_TO_PART_3 <- TRUE # bool flag to comment out the ~600 lines of part 1 and 2
...
plot(
  ...
  # - nodes appear to have a semitransparent opacity set
  vertex.color = adjustcolor(vertex_colors, alpha.f = 0.75), # VERTEX ALPHA HERE
  vertex.shape = vertex_shapes,
  ...
  edge.width = 3 * scales::rescale(
    E(gangs)$weight,
    to = c(min(E(gangs)$weight),
           max(E(gangs)$weight)
           )
    ), # scale factor of 3? Seems enough
  
  #  - edges also appear to have some transparency set
  edge.color = adjustcolor(edge_colors, alpha.f = 0.75), # EDGE ALPHA HERE
  edge.lty = edge_line_types,
...
legend_TR_base <- legend(
  ...
  pt.cex = scales::rescale(
    ...
    to = c(0, 1) # normalize to between 0-1
  ),
...
symbols(x, y, # symbols coordinates
        circles = arrest_count_cutoffs / 200, # vector of circle radii - Circles of radius zero are plotted at radius one pixel
        inches = FALSE, # units are taken to be those of the appropriate axes, 
        add = TRUE, # adding to existing plot, yep
        bg = "orange" # background color of orange? for now TODO: make these equal to the symbol color palette matching birthplace, can be lazy and just pulled color of the first entry in the palette
