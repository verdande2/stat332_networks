...
plot_params <- list(
  ...
  edge.arrow.size = edge.widths * 0.35 (this will dynamically adjust them)
...
v_pal <- setNames(
  brewer.pal(length(v_levels), "Dark2"),
  v_levels
)
...
#  VERTEX SHAPES
# ------------------------
v_s_vals <-  V(g)$type.label
shape_levels <- levels(as.factor(v_s_vals))
...
lty_palette <- c(1, 2,3,4,5,6)  # or however many you want
e_lty_pal <- setNames(
...
plot(
  ...
  vertex.size = scales::rescale(V(g)$audience.size, to = c(5, 25)),
  ...
  edge.arrow.size = scales::rescale(E(g)$weight, to = c(0.5, 3))*0.15,
  edge.arrow.mode = 1
)
