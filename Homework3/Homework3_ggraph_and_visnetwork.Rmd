...
DUMP_FONT_INFO_TO_FILE <- TRUE # true to dump font import log to file
FONT_INFO_LOG <- "extrafonts.log"

SKIP_AHEAD <- FALSE # skips ahead and truncates off the demo code/markdown (uses a TeX comment to omit output) # TODO disable this, not needed
```


```{r setup_fonts, include = FALSE}
# FOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOONTS
...
if (DUMP_FONT_INFO_TO_FILE){
  font_log_filepath = FONT_INFO_LOG
} else {
  font_log_filepath = NULL
}
# loadfonts(device="win")
# eh, who knows, maybe we'll need it at some point
capture.output({
  extrafont::font_import(prompt = FALSE)
  extrafont::loadfonts(device = "win", quiet = FALSE)
}, file = font_log_filepath, append = FALSE, type = c("output", "message"))
...
handedness <- c("U", "R", "L") # should be one of L, R, U
shape_codes <- c(16, 15, 22)
shape_vals <- shape_codes[seq_along(handedness)]
shape_map <- setNames(shape_vals, handedness)
...
```{r plot_visnetwork_graph, eval=FALSE}
...
```{r try_the_visigraph_plot, eval=FALSE}
...
```{r final_helper_functions, eval=FALSE}
...
```{r plot_visnetwork_graph_take_two, eval=FALSE}
