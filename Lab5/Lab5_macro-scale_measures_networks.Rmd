...
show_k_core_plots(g) # function definition moved to graph.R
```

```{r do_the_same_for_k_shell_plots}
show_k_core_shell_plots(g) # function definition moved to graph.R
```
...
Why yes, yes I did copy/paste that from Lab 4 and didn't want to reformat that blob of text. You worked with this same `graphml` object last week. 

**I'd like you to use a variety of the above algorithms to try to discover meso-scale structural patterns within this dataset.**
...
```{r}
cliques(g, min = clique_num(g)-1)
```

22 of one less than that. Alright!

So, of max (clique) length 9, we have 2 cliques.

**What communities can you find using density methods and how could this be helpful at finding the key operations?**

```{r communities}
g_leiden <- cluster_leiden(g, weights=E(g)$weight)
plot(g_leiden, g)
membership(g_leiden)
length(g_leiden)
sizes(g_leiden)
...
```{r}
g_louvain <- cluster_louvain(g, weights=E(g)$weight)
plot(g_louvain, g)
membership(g_louvain)
length(g_louvain)
sizes(g_louvain)
algorithm(g_louvain)
```
...
```{r}
g_cen <- cluster_edge_betweenness(g,
                         weights = E(g)$distance_weight)
```
