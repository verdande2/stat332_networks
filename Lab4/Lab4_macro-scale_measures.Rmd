...
Laplacian <- laplacian_matrix(g, weights = E(g)$weight)
eigenvalues <- eigen(Laplacian, symmetric = TRUE, only.values = TRUE)$values
# lambda_2 <- eigenvalues[length(eigenvalues) - 1] # note that this does not take multiplicity into account TODO not sure which is which
lambda_2 <- unique(eigenvalues)[length(unique(eigenvalues)) - 1] # note that this does not take multiplicity into account
...
cat(paste0(
  ...
  sum(igraph::strength(g, mode = "all") == 0),
