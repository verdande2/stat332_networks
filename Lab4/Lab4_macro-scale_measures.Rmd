...
Well, it's all connected, weighed, and doesn't have any loops or multiply-connected edges, so that makes things simpler. 

```{r largest_component}
# probably unneeded
largest_component_of_g <- largest_component(g)
largest_component_of_g

```

```{r precompute_degrees_and_strengths}
...
# had to make the graph undirected to calculate the laplacian below, else get:
...
L <- laplacian_matrix(g_undirected, weights = E(g)$weight, normalization = "symmetric")
# L <- laplacian_matrix(g, weights = E(g)$weight, normalization = "symmetric") # I'm not sure why this doesn't work as is
...
```{r}
# edge weights represent number of communications, so I'd assume we should minimize shortest paths based on reciprocal of the weight
