...
df <- df |> select(-...1) # select all BUT that first col (...1)
...
p <- ggraph(g_actors, layout = "lgl") +
  ...
  # custom colors? maybe? or too obnoxious?
  geom_node_text(
    aes(
      label = Actor
      ),
  ...
  geom_edge_arc(
    aes(
      color = ifelse(Role == "Command Team", "red", "black"), # denote command team
      # linetype = type, # this line /also/ triggers the noted error below! Argh!
      width = weight
      ),
    alpha = 0.4,
