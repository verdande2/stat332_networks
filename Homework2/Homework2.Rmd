...
print_big_fancy_summary_table <- function(graph) {
  ...
  print(big_fancy_igraph_table |> gt())
}
...
print_metadata_summary_table <- function(graph) {
  ...
  colnames(tbl_TF) <- c("Test", "Value")
  print(tbl_TF |> 
    gt() |> 
    fmt_tf(columns = c(Value), tf_style = "yes-no")
  )
  ...
  colnames(tbl_components) <- c("Test", "Value")
  print(tbl_components |> 
    gt() |> 
    fmt_number(columns = c(Value), decimals = 0, use_seps = TRUE)
  )
}
```
...
print_metadata_summary_w_compare <- function(g) {
  ...
  colnames(tbl_TF) <- c("Test", "Value", "Expected")
  print(tbl_TF |> 
    gt() |> 
    fmt_tf(columns = c(Value, Expected), tf_style = "yes-no")
  )
  ...
  colnames(tbl_components) <- c("Test", "Value", "Expected")
  
  
  print(tbl_components |> 
    gt() |> 
    fmt_number(columns = c(Value, Expected), decimals = 0, use_seps = TRUE)
  )
}
