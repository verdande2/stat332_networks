...
```{r}
length(unique(edge_attr(g)$Carrier))
```
$118$ carriers, eesh, okay. Maybe that's not even the best attribute to subset on. Let's look at the biggest airports instead, that may be a better starting point.

```{r}
length(unique(vertex_attr(g)$name))
```

$755$ airport connections listed in this, eesh, well. Let's see if we can subset these reasonably. Maybe the top 25? 

```{r}
deg <- strength(g, weights = V(g)$Departures) # calculate degrees for whole graph

head(sort(deg, decreasing = TRUE), n = 25) # pull top 25 sorted by connecting flights
```
