...
SKIP_TO_PART_3 <- FALSE # bool flag to comment out the ~600 lines of part 1 and 2, set to FALSE to output parts 1 and 2
```
...
![Goal network image for this lab](Example Goal.png)
...
The variable `graph` is simply the name of the graph. `v` is the list of nodes that we will perform the calculations for. Obviously, we will want to calculate the degree for all vertices and `V(graph)` is the default so we don't even need to do anything there. There are loops in the graph, IF we want them to count. And now is our first decision point. Is it important if a media outlet links or references themselves? Does this add value to their trustworthiness, authority, value, or anything else? If not, perhaps we should remove the loops before any further analysis. (Our goal picture does have a loop represented, so we are going to leave them in, but we will investigate filtering out loops in a moment. Finally, `normalized` just divides every value by `n-1` which doesn't serve much purpose for degree (for other measures it's a lot more meaningful.)
...
You might notice that the information for the title, subtitle, caption, (and eventually our legends appear outside of the `plot()` function. These are actually added onto the canvas after the graph object has been created.
...
vertex_levels <- vertex_labels |> unique() |> sort() # will look like c("1", "2", "3", "4")
...
if (PNG) {
  png("LondonGangNetwork.png", width = 1920, height = 1200, res = 200) # saving to file
}
...
plot(
  ...
  vertex.label.dist = 0,
  edge.width = 2 * E(gangs)$weight, # scale factor of 2? Seems enough

  edge.color = adjustcolor(edge_colors, alpha.f = 0.5),
  edge.lty = edge_line_types,
  edge.curved = 0, # straight lines
  edge.label = "", # edges are unlabeled for this plot
...
legend(
  "bottomleft",
  title = "Edge Weight",
  legend = E(gangs)$weight |> unique() |> sort(), # note, not multiplied by factor of 2
  lwd = 2 * E(gangs)$weight |> unique() |> sort(),
  cex = 1.0 # scale
)
...
legend(
  ...
  legend = V(gangs)$Birthplace |> unique() |> sort(), # should be birthplace str == one of c("1", "2", ...)
  cex = 1.0, # scale
  fill = adjustcolor(vertex_colors[names(vertex_colors) |> unique() |> sort()], alpha.f = 0.75) # had to do a little black magic to make colors happen
)
...
```{r tkplot_testing}
# Well, I /was/ going to play with tkplot and try to visually adjust the vertex spacing and such, but it appears that tkplot is ... rather useless...
#id <- tkplot(gangs)
```


![London Gang Network](LondonGangNetwork.png)
