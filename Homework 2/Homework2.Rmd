...
# first edge connecting... how do I determine who was first? First seen in the edge list I guess? alphabetical order? maybe A->B is "greater" than B->A? Sort it alphabetically, that sounds workable...
...
data <- data.frame(
  from = c("A","E","E"), 
  to = c("B","D","D"), 
  name = c("S2", "S2", "S3"), # TODO think about this... maybe ask joe...
  weight = c(0,0,0)
  )

igraph::add_edges(
  graph,
  unlist(data[, c("from", "to")])
  )

graph <- set_edge_attr(graph, name = "name", value = data[, c("name")])

# this order won't work. I need to assign the name attribute for edges earlier, with the "first" edge being S1, second S2 and so on, but this is a bidirectional graph, so it's not like A->B is measurable against B->A 
# lazy programmer method would be to loop over all nodes, get their connected edges, and then number them as they come and deal with it later. I think that seems prudent.
```
