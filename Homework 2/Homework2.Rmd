...
# TODO resume here, double check graph vs g for variable names

# actually add the edges to the graph
graph <- igraph::add_edges(
  graph,
  unlist(new_edges[, c("from", "to")]), # hoping to get a list of (from1, to1, from2, to2, ...)
...
E(graph)$weight = runif(length(E(graph))) # all edges
```
...
Excellent. Random weights in [0,1] assigned to edge weights! Now, verifying the graph should have no isolates:
...
Now the graph is done, we'll write the edge list to file.
...
tbl_nodes <- data.table(
  ...
  type = lapply(V(graph)$fullname, fruit_or_veg) # type will be Fruit, Vegetable, or Unknown Frui-egetable! in the case of an unknown string
)
tbl_nodes |> gt()
...
node_type_dt <- data.table(
  ...
  type = unlist(lapply(V(graph)$fullname, fruit_or_veg)) # FUCK this line of code
...
print_metadata_summary_table(g)
```
...
# At vendor/cigraph/src/graph/iterators.c:831 : Cannot create iterator, invalid vertex ID. Invalid vertex ID
# TODO debug this....
```
...
```{r}
print_metadata_summary_w_compare <- function(g) {
  tbl <- data.table(
    Label <- c(
      "Directed?",
      "Weighted?",
  ...
  colnames(tbl) <- c("Test", "Value", "Expected")
  tbl |> gt()
}


print_metadata_summary_w_compare(g)
# TODO I feel like this is an appropriate place to employ tests and asserts properly, but I need to delve more into testing in R
# need moar asssert()s
```
...
graph <- set_graph_attr(g, name = "Weighted", value = TRUE)
graph <- set_graph_attr(g, name = "Directed", value = TRUE)
```
...
#g <- igraph::set_vertex_attr(g, "name", value = node_name_list)
#g <- igraph::set_vertex_attr(g, "type", value = node_type_list)
...
print_metadata_summary_table(g) # to compare to the default igraph summary (also has is_weighted call in there)
```
