...
```{r make_CSVs_if_needed, echo = FALSE}
# TODO suppress R console and data.frame output
rmarkdown::render("make_graph_csvs.Rmd") # will spit out edge_list and node_list in CWD, and a html file with verbose creation output
```
...
plot(
  ...
  vertex.sizes = 10, #scales::rescale(V(g)$audience.size, to = c(5, 25)),
...
plot(
  ...
  vertex.size = scales::rescale(V(g)$degree_in, c(10, 50)),
  ...
  vertex.label.cex = scales::rescale(V(g)$degree_in, c(1, 3)),
  vertex.label.dist = 0.5, # ctr
...
# Add a title
title_text <- "Bipartite Graph"
mtext(side = 3, line = 1, at = 0, adj = 0.5, cex = 1, title_text) # 3 = top, adj = 0.5 is centered

# subtitle
subtitle = "Nodes A-H represent fruits and vegetables. The weighted edges represent ???."
mtext(side = 1, line = 1, at = -2, adj = 0, cex = 0.7, subtitle)
...
# legends
# indicates vertex size being based on in-degree
v_levels <- 1:7 # legend levels
legend(
  "topright",
  title = "Vertex Size",
  legend = v_levels,
  ...
  border = NA,
  bty = "n",
  cex = scales::rescale(v_levels, c(0.2, 1.2)) # need to scale based on in_degree of node
) # TODO fix the scaling of the legend!
```
