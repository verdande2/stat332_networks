...
# graph <- g1 + g2
# 
# # now we tidy things up, make sure things are properly combined, no duplicate attrs, etc
# 
# # graph now has some duplicated attributes like name_1 and name_2, lazily remove those attrs
# graph <- igraph::delete_graph_attr(graph, "name_1")
# graph <- igraph::delete_graph_attr(graph, "name_2")
# graph <- igraph::delete_graph_attr(graph, "loops_1")
# graph <- igraph::delete_graph_attr(graph, "loops_2")
# graph <- igraph::delete_edge_attr(graph, "name_1")
# graph <- igraph::delete_edge_attr(graph, "name_2")
# 
# # now, I made those two subgraphs with loops = FALSE, but igraph still added the loops attr to the graph, then when I merged them, igraph renamed them loops_1 and loops_2, I don't /think/ I need the loops graph attr, but just in case, add it back in
# graph <- igraph::set_graph_attr(graph, "loops", FALSE)
```
...
graph
plot(graph) # TODO why are there so many god damn curved edges when this renders?!?!?!??
```
...
Let's verify the graph now has edge attribute "name" properly set:

```{r verify_edge_attr_name}
E(graph)$name
```

Cool. On to adding the edge attribute "weight" to a uniformly distributed random decimal in [0, 1]:
...
testit::assert("All weights should be in [0, 1].", {
  sum(E(graph)$weight > 1) + sum(E(graph)$graph < 0) == 0
})
```
...
tbl_nodes <- data.table(
  name = V(graph)$fullname, # pull the names directly from the vertices of the graph (but these names are just letters, we need the full string, so use fullname)
  # Apple, Banana, Carrot, Date, Eggplant, Fig, Grapes, and Horseradish
  type = lapply(V(graph)$fullname, fruit_or_veg) # type will be Fruit, Vegetable, or Unknown Frui-egetable! in the case of an unknown string # TODO look up exceptions/error handling/parameter typechecking/validation/etc in R
)
tbl_nodes |> gt()
...
node_type_dt <- data.table(
  name = V(graph)$name, # Letter names
  
  # TODO: figure out why this next line is coming out as nested lists
  type = lapply(unlist(V(graph)$fullname), fruit_or_veg) # type will be Fruit, Vegetable, or Unknown Frui-egetable! in the case of an unknown string # TODO look up exceptions/error handling/parameter typechecking/validation/etc in R
)
