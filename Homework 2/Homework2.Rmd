...
# => strongly connected means bidirectional paths between all pairs, so F/G/H are all completely dense in their subgraph
g2_vertex_labels <- c("F", "G", "H")
g2 <- igraph::make_full_graph(length(g2_vertex_labels), 
                      directed = TRUE, 
                      loops = FALSE
                      )
V(g2)$name <- g2_vertex_labels # assigning the lettered labels to the name of each vertex in the subgraph g2
```

Now, a quick double check as usual:
```{r verify_second_component_creation}
V(g2) # to see labeled vertices
E(g2) # to see label vertices in edge pairs
plot(g2)
```
...
plot(graph)
```
Well, that is shaping up nicely!

And now we add the remaining edges to connect the two subgraphs:
...
# (A, B, blah, blah) <- original edge in graph
# (A, B, blah2, blah2)
# as well as
...
# first edge connecting... how do I determine who was first? First seen in the edge list I guess? alphabetical order? maybe A->B is "greater" than B->A? Sort it alphabetically, that sounds workable... I guess the instructions probably assumed there'd be an "order" in the csv in terms of line numbers, that makes sense... or is the purpose more to aim for each edge with shared start/end points to have a unique name, that makes more sense in my head, so you can identify _which_ A->B edge from the two of them.

# graph is our combined igraph object, let's add those 3 edges and label them!
# first, setup a df with the edge data
data <- data.frame(
  from = c("A","E","E"), 
  to = c("B","D","D"), 
  name = c("S2", "S2", "S3"), # TODO this is not right ... think about this... maybe ask joe...
  weight = c(0,0,0)
  )
...
#graph <- set_edge_attr(graph, name = "name", value = data[, c("name")]) # note to self: set_edge_attr() sets ALL nodes in one fell swoop, so the dimensions of value vector must match length(E(graph))
...
nodes_filename <- "nodes_list.csv"
tbl_nodes <- data.table(
  name = V(graph)$name, # pull the names directly from the vertices of the graph
  # Apple, Banana, Carrot, Date, Eggplant, Fig, Grapes, and Horseradish
  type = fruit_or_veg(V(graph)$name) # type will be Fruit, Vegetable, or Unknown Frui-egetable! in the case of an unknown string # TODO look up exceptions/error handling/parameter typechecking/validation/etc in R
)
tbl_nodes |> gt()
```
...
# okay, there are 4 columns in the edgelist csv: to, from (both of which are nodes), weight (for the edge), and name (of the edge)
# therefore, my goal is to add the edge weights and the edge names
name_list <- c()
igraph::set_edge_attr(graph, "name", name_list)
igraph::set_edge_attr(graph, "Weight", weight_list)
```

Let's verify that those attributes got bolted onto our igraph object:
...
nodes_list <- readr::read_csv("node_list.csv", header = TRUE, sep = ",")
name_list <- nodes_list[, c("name")]
type_list <- nodes_list[, c("type")]
```
...
igraph::set_vertex_attr(graph, "name", name_list)
```

Let's verify we see name and weight in our igraph:
...
```{r}
# using default summary and my helper function to barf out a nicely formatted gt table of various metrics for the graph and compare
igraph::summary(graph)
print("----------------------------------------------")
