...
# => multigraph means more than one edge between a pair of vertices, so there are two entries in the edge list like: 
# (A, B, blah, blah)
# (A, B, blah2, blah2)
# as well as
# (E, D, blah, blah)
...
# first edge connecting... how do I determine who was first? First seen in the edge list I guess?

# ig <- set_edge_attr(ig, name = "name", value = v)
```
...
```{r isolates_check}
# The graph should have no isolates
ig_degrees <- igraph::degree(ig) # gets all the vertex's degrees in col vector
num_of_isolates <- sum(ig_degrees == 0) # sum of all the entries that are ==0 aka isolated
...
data.table(
  name = V(ig)$name, # pull the names directly from the vertices of the graph
  # Apple, Banana, Carrot, Date, Eggplant, Fig, Grapes, and Horseradish
  type = fruit_or_veg(V(ig)$name) # type will be Fruit, Vegetable, or Unknown Frui-egetable! in the case of an unknown string # TODO look up exceptions/error handling/parameter typechecking/validation/etc in R
)
```
...
testit::assert(paste0("ig object should be of class: igraph, but returned class: ",class(ig)), {
  class(ig) == "igraph"
})
```
...
print_metadata_summary_table(ig)
...
print_metadata_summary_w_compare(ig)
...
name_list <- c()
igraph::set_edge_attr(ig, "name", name_list)
igraph::set_edge_attr(ig, "Weight", weight_list)
```
...
igraph::set_vertex_attr(ig, "name", name_list)
```
...
```{r}
# using default summaery and my helper function to barf out a nicely formatted gt table of various metrics for the graph and compare
igraph::summary(ig)
print("----------------------------------------------")
print_metadata_summary_table(ig) # to compare to the default igraph summary (also has is_weighted call in there)
```
